...
class Functions
{
  ...
  public function checkURL($index = false)
  {
    ...
    if ($index)
      array_push($urls, "index.php");
    else if (array_search('index.php', $urls))
      $urls = array_diff($urls, ["index.php"]);
    if (in_array($url, $urls))
      $this->redirect($configBase, 301);
  }
...
*/
  ...
  public function checkPermission($com = '', $act = '', $type = '', $array = null, $case = '')
  {
    ...
    if ($case == 'phrase-1') {
      if ($type != '')
        $str .= '_' . $type;
      if (!in_array($str, $_SESSION[$loginAdmin]['permissions']))
        return true;
      else
        return false;
      ...
      if ($array) {
        ...
        foreach ($array as $key => $value) {
          if (!in_array($str . "_" . $key, $_SESSION[$loginAdmin]['permissions']))
            $count++;
        }
  ...
  public function checkRole()
  {
    ...
    if ((!empty($_SESSION[$loginAdmin]['role']) && $_SESSION[$loginAdmin]['role'] == 3) || !empty($config['website']['debug-developer']))
      return false;
    else
      return true;
  }
  ...
  public function filterUniqueBrands($datas, $type, $lang)
  {
    $brands = [];
    foreach ($datas as $v) {
      $brand = $this->d->rawQueryOne("select name$lang, slug$lang, id from #_product_brand where id = ? and type = ? and find_in_set('hienthi',status)", array($v['id_brand'], $type));
      if ($v['id_brand'] == $brand['id']) {
        array_push($brands, $v['id_brand'] . '-' . $brand['name' . $lang]);
      }
    }
  ...
  /* Transfer */
  public function transfer1($msg = '', $page = '', $numb = true)
  {
    global $configBase;
    ...
    include("./pages/layout/transfer.php");
    exit();
  }
  ...
  public function uploadImage($file = '', $extension = '', $folder = '', $newname = '')
  {
    ...
    if (isset($_FILES[$file]) && !$_FILES[$file]['error']) {
      ...
      if ($image_w > $maxWidth)
        $this->smartResizeImage($folder . $_FILES[$file]['name'], null, $maxWidth, $maxHeight, true);
  ...
  public function removeDir($dirname = '')
  {
    ...
    if (!isset($dir_handle) || $dir_handle == false)
      return false;
  ...
  public function createThumb($width_thumb = 0, $height_thumb = 0, $zoom_crop = '1', $src = '', $watermark = null, $path = THUMBS, $preview = false, $args = array(), $quality = 100)
  {
    ...
    if (!file_exists($upload_dir)) if (!mkdir($upload_dir, 0777, true))
    ...
    if (!empty($watermark['status']) && strpos('hienthi', $watermark['status']) !== false) {
      ...
      if (min($_new_width, $_new_height) <= 300)
        $per_scale = $per_small_scale;
    ...
    if ($upload_dir) {
      if ($func == 'imagejpeg')
        $func($canvas, $upload_dir . $image_name, 100);
      else
        $func($canvas, $upload_dir . $image_name, floor($quality * 0.09));
    }
  ...
  public function orderStatus($status = 0)
  {
    ...
    foreach ($row as $v) {
      if (isset($_REQUEST['order_status']) && ($v["id"] == (int) $_REQUEST['order_status']) || ($v["id"] == $status))
        $selected = "selected";
      else
        $selected = "";
  ...
  public function getTags($id = 0, $element = '', $table = '', $type = '')
  {
    ...
    for ($i = 0; $i < count($row_tags); $i++) {
      if (!empty($temps)) {
        if (in_array($row_tags[$i]['id'], $temps))
          $selected = 'selected="selected"';
        else
          $selected = '';
      } else {
