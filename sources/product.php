...
if ($id != '') {
  $criterias = $d->rawQuery("select name$lang, photo from #_news where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('tieuchi'));
  /* Lấy sản phẩm detail */
  $rowDetail = $d->rawQueryOne("select availability, type, id, name$lang, slugvi, slugen, desc$lang, content$lang, code, view, id_brand, id_list, id_cat, id_item, id_sub, photo, options, discount, sale_price, regular_price from #_product where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));
  ...
  $sql = "select photo, name$lang, slugvi, slugen, sale_price, regular_price, discount, id, id_brand from #_product where $where order by numb,id desc";
  ...
  if ($func->isAjax() && !empty($_GET['quickview'])) {
    $quickview = true;
    include PAGES . "product/product_detail_tpl.php";
    exit;
  }
} else if ($idl != '') {
  /* Lấy cấp 1 detail */
  ...
  $sql = "select photo, name$lang, slugvi, slugen, sale_price, regular_price, discount, id, id_brand from #_product where $where order by numb,id desc $limit";
  $product = $d->rawQuery($sql, $params);
