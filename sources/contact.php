...
if (!empty($_POST['submit-contact'])) {
  ...
  if (($scoreCaptcha >= 0.5 && $actionCaptcha == 'contact') || $testCaptcha == true) {
    ...
    if ($emailer->send("admin", $arrayEmail, $subject, $message, $file)) {
      ...
      if ($emailer->send("customer", $arrayEmail, $subject, $message, $file)) $func->transfer1(guilienhethanhcong, $configBase);
    } else $func->transfer1(guilienhethatbai, $configBase, false);
  } else {
